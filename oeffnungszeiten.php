...
/*
  ...
  Plugin URI: http://www.all-opening-times.co.uk
  Description: Opening Times for your Website
  Version: 1.0.0
  Author: all-opening-times.co.uk
 */
	...
	class OeffnungszeitenScriptWidget extends WP_Widget
	{
		...
		function OeffnungszeitenScriptWidget()
		{
			$widget_ops = array('classname' => 'widget_oeffnungszeiten_script',
				'description' => __("Your Opening Times Search Widget"));
			$this->WP_Widget('oeffnungszeiten_script', 'Opening Times', $widget_ops);
		}
...
//			$title = apply_filters('widget_title', $instance['title']);
			...
			echo $before_widget;
			?>
			<!-- all-opening-times.co.uk -->
			<link rel="stylesheet" type="text/css" href="http://www.all-opening-times.co.uk/rpc/widget/templates/style/searchwidget.css">
			<script type="text/javascript" src="http://www.all-opening-times.co.uk/rpc/widget/templates/js/widgetbranding.js"></script>
			<div id="oe-searchwidget" style="background: <?php echo get_option('textbgcolor', '#C7E0FF'); ?>; color: <?php echo get_option('textcolor', '#000000'); ?>;" class="backgroundStyle">
				<div id="oe-searchwidget-title" style="background: <?php echo get_option('titlebgcolor', '#489BFF'); ?>; color: <?php echo get_option('titlecolor', '#FFFFFF'); ?>;">Find Opening Times</div>
				<iframe src="http://www.all-opening-times.co.uk/rpc/widget/widget.php?wo=<?php echo urlencode(get_option('widgetwo', '')) ?>&was=<?php echo urlencode(get_option('widgetwas', '')) ?>&textcolor=<?php echo urlencode(get_option('textcolor', '#000000')) ?>&textbgcolor=<?php echo urlencode(get_option('textbgcolor', '#C7E0FF')) ?>&buttoncolor=<?php echo urlencode(get_option('buttoncolor', '#FFFFFF')) ?>&buttonbgcolor=<?php echo urlencode(get_option('buttonbgcolor', '#489BFF')) ?>" width="150" height="200" frameborder="0" style="overflow: hidden;"></iframe>
...
function getWidgetBranding(wo, was, color) {
		...
		var brandingtext = "";
		if(wo != "" && was == "") {
			brandingtext = 'Store Hours in <a href="http://www.all-opening-times.co.uk/city/'+ucFirst(wo)+'-1.html" style="color: '+color+'; font-size: 10px;">'+ucFirst(wo)+'</a>. all-opening-times.co.uk .';
		}else if(wo == "" && was != "") {
			brandingtext = 'StoreHours '+ucFirst(was)+'. <a href="http://www.all-opening-times.co.uk/" style="color: '+color+'; font-size: 10px;">all-opening-times.co.uk</a>';
		}else if(wo != "" && was != "") {
			brandingtext = '<a href="http://www.all-opening-times.co.uk/search/store-'+ucFirst(wo)+'-30+km-'+ucFirst(was)+'-1.html" style="color: '+color+'; font-size: 10px;">&Ouml;ffnungszeiten '+ucFirst(was)+' in '+ucFirst(wo)+'</a>. Ein Service von &Ouml;ffnungszeitenbuch.';
		}else{
			brandingtext = 'Find Store Hours Fast! By <a href="http://www.all-opening-times.co.uk/" style="color: '+color+'; font-size: 10px;">all-opening-times.co.uk</a>.';
		}
